...
3. `docs/AUTOMATION_OPERATING_POLICY.md`의 온보딩 체크리스트에 따라
   ...
   보고 기준을 정한다.
   sandbox-vs-real-network evidence 기준, plan index/TODO reconciliation,
   spec/design alignment ownership도 함께 확정한다.
...
5. 장기 정책, 표준, 아키텍처, 검증 체계, 배포/보안/데이터 작업은
   plan-first interview 후 진행한다. 새 기획, 계획, 전략, 아키텍처, 운영
   판단 문서는 한국어를 기본으로 작성하되 명령어, 파일 경로, 상태값,
   schema field, 코드 식별자는 영어를 유지한다. 기존 영문 문서는 사용자
   요청이나 해당 문서 정비 범위가 없으면 그대로 둔다.
...
상태이면 활성 주관자의 subagent substitute가 그 lane을 대체 리뷰로
담당한다. 이 대체 리뷰는 정규 외부 리뷰가 아니라 항상 degraded coverage다.
usable verdict와 `Direct File Inspection` 증거가 있어도 proceed_degraded(degraded
trust)로 보고하고, 없으면 blocked로 남긴다.
...
transient disable(usage/세션/주간/quota/rate limit·network)은
`REVIEW_REVIEWER_DISABLE_COOLDOWN_SECONDS`(기본 1800) 경과 후 다음 실행에서
자동 복구되고, persistent/미분류 disable은 사용자가
`RESET_DISABLED_AI_REVIEWERS=claude|gemini|all`로 복구하기 전까지 스킵된다.
...
대체 리뷰로 담당한다. 이 대체는 정규 외부 리뷰가 아니라 항상 degraded
coverage이며, usable verdict·직접 파일 확인이 있어도 proceed_degraded, 없으면
blocked로 보고한다.
