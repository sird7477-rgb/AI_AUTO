...
User-defined completion criteria are immutable acceptance scope; never narrow
them. An intermediate fail-closed safety gate (e.g. a no-order/no-candidate
guard) is not completion: complete only by proving the user's deliverable with
its required evidence, or by an explicit no-result final report carrying every
required evidence item (candidates, backtests, fallback loops, AI unanimity).
