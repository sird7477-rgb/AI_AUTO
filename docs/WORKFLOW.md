...
프로젝트 초기 환경 구축에서는 `ai-auto setup`으로 전역 AI_AUTO 모드를
도입한다(훅 심 설치 + .omx ignore + 잔존 프레임워크 파일 정리, 멱등). 이미
도입된 프로젝트는 README/docs/scripts/package 파일을 먼저 확인한다. 이후
...
Claude 또는 Gemini가 세션 제한, 주간 제한, quota/rate limit 등으로 응답할 수 없으면 해당 리뷰어는 `.omx/reviewer-state/`에 disabled 상태로 기록된다. transient disable(usage/세션/주간/quota/rate limit·network)은 `REVIEW_REVIEWER_DISABLE_COOLDOWN_SECONDS`(기본 1800) 경과 후 다음 실행에서 자동 복구되고, persistent/미분류 disable은 사용자가 `RESET_DISABLED_AI_REVIEWERS=claude|gemini|all`로 복구하기 전까지 스킵된다.
