...
- U3.1+U3.2 diff scope policy를 review-gate 실제 caller로 배선 완료. scope가
  ...
  code/script/template/guidance/unknown/guarded diff는 full review 유지.
- U3.3 `REVIEW_UNTRACKED_ALLOWLIST`를 tracked diff scope에서 자동 도출하도록
  완료. untracked-only 상태는 기존처럼 material untracked를 모두 blocking으로
  유지하고, tracked diff가 있으면 같은 스코프의 untracked만 blocking으로 둔다.
- U4.1 one-writer-per-working-tree 컨벤션을 root/template `AGENTS.md`와
  `docs/WORKFLOW.md`에 명시 완료.
