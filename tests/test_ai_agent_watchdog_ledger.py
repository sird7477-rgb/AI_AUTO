...
def test_record_not_before_gate_and_due(tmp_path):
    wd = _wd(tmp_path)
    assert wd("ledger-record", "--key", "k", "--payload", "redo", "--reset-epoch", "2000000000", "--max-retries", "2").returncode == 0
...
def test_idempotent_by_key(tmp_path):
    wd = _wd(tmp_path)
    wd("ledger-record", "--key", "same", "--reset-epoch", "2000000000")
    wd("ledger-record", "--key", "same", "--reset-epoch", "2000000000")
...
def test_max_retries_exhaustion(tmp_path):
    wd = _wd(tmp_path)
    wd("ledger-record", "--key", "k", "--reset-epoch", "2000000000", "--max-retries", "2")
...
def test_done_removes_from_due(tmp_path):
    wd = _wd(tmp_path)
    wd("ledger-record", "--key", "k", "--reset-epoch", "2000000000")
