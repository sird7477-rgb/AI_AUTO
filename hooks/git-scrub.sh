...
#     (incl. GIT_CONFIG_PARAMETERS — git's `-c` serialization env, honored on EVERY git
#      command, additive to + HIGHER precedence than KEY/VALUE; NOT reached by the KEY_*/VALUE_*
#      glob loop below, so it is unset by name — else it defeats even our core.fsmonitor='' re-pin)
