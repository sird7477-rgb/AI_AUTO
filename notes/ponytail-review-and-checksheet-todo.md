...
**정정된 사실(초기 분석 오류):** 과거 벤더링 미러(프로젝트로 복사되던 scripts/ 사본) 바이트동일을
*강제하는 게이트는 없었음*(parity 테스트는 substring, 동일은 관행). 실제 강제는 버전범프뿐이었고,
globalization에서 벤더링 미러·버전범프 모델 자체가 제거됨.
