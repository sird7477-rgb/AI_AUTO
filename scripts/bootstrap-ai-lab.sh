...
if [ -x "${ROOT}/tools/ai-register" ]; then
  ...
  say_fail "ai-register source helper is missing or not executable"
...
if [ -x "${ROOT}/tools/ai-domain-pack" ]; then
  say_pass "ai-domain-pack source helper is executable"
else
  say_fail "ai-domain-pack source helper is missing or not executable"
...
if [ -x "${ROOT}/tools/ai-refactor-scan" ]; then
  ...
  say_fail "ai-refactor-scan source helper is missing or not executable"
  suggest "chmod +x ${ROOT}/tools/ai-refactor-scan"
fi

if [ -x "${ROOT}/tools/ai-rebuild-plan" ]; then
  say_pass "ai-rebuild-plan source helper is executable"
else
  say_fail "ai-rebuild-plan source helper is missing or not executable"
...
if [ -x "${ROOT}/tools/knowledge-collect" ]; then
  ...
  say_fail "knowledge-collect source helper is missing or not executable"
  suggest "chmod +x ${ROOT}/tools/knowledge-collect"
fi
...
if [ -n "$HOME_DIR" ] && [ "$HOME_READY" -eq 1 ]; then
  ...
  ensure_link "${HOME_DIR}/bin/ai-worktree" "${ROOT}/tools/ai-worktree"
